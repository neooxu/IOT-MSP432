...
void drv_board_test(void)
{
	...
	if ( true == GPIO_getInputPinValue(GPIO_PORT_P4, GPIO_PIN6) ) return;
	...
	while (1) {
		/* Test SHT20 sensor */
		sprintf(sensor_display[0], "Board Test, %d/%d", GPIO_getInputPinValue(GPIO_PORT_P5, GPIO_PIN5), GPIO_getInputPinValue(GPIO_PORT_P5, GPIO_PIN4));
